...
- **No dedicated test for shift series creation.** The claim and shift-edit tests cover the pieces the series feature is built on top of, but the day-of-week generation and whole-series delete aren't directly tested yet.

## One thing I'd do differently with more time

Earlier drafts of this doc had the edit-time re-validation gap sitting right here, that's fixed now (see "Editing the shifts" above), and the recurring shifts stretch goal is also actually built now, not just designed on paper. So I want to name the next thing I'd actually prioritize rather than leave a stale answer in place. It'd be closing the manager-self-claim gap noted above: right now the only thing stopping a manager from self-claiming a shift is that the frontend never shows them as an option in the assign picker. That's the same shape of problem the edit-revalidation gap used to be, a rule that's true in practice only because of what the UI happens to render, not because the server actually enforces it. Given how much of this project's design was about making sure the *server*, not the client, is the one place a business rule can't be bypassed, leaving this one exception in is the thing I'm least comfortable with right now.